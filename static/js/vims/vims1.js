...
/*Step 3) Change: vims1 */
var vims1;  //declared outside function for use in layer control
window.addEventListener('DOMContentLoaded', init)
if (window.location.hash === "#cluster") {
	var vims1 = new L.MarkerClusterGroup();  	// Set up cluster group
} else {
	var vims1 = new L.LayerGroup();  	// Otherwise set up normal marker group
}
...
//Step 7) Change if(current.type ==) { vims1.addLayer(layer_marker); }
function loadMarkersToMap(data) {
  for(var i = 0; i < data.length; i++) {
    ...
    if(current.type == '1') { vims1.addLayer(layer_marker); }
	}
