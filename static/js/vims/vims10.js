...
/*Step 3) Change: vims10 */
var vims10;  //declared outside function for use in layer control
window.addEventListener('DOMContentLoaded', init)
if (window.location.hash === "#cluster") {
	var vims10 = new L.MarkerClusterGroup();  	// Set up cluster group
} else {
	var vims10 = new L.LayerGroup();  	// Otherwise set up normal marker group
}
...
//Step 7) Change if(current.type ==) { vims10.addLayer(layer_marker); }
function loadMarkersToMap(data) {
  for(var i = 0; i < data.length; i++) {
    ...
    if(current.type == '10') { vims10.addLayer(layer_marker); }
	}
