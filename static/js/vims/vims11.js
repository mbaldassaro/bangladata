...
/*Step 3) Change: vims11 */
var vims11;  //declared outside function for use in layer control
window.addEventListener('DOMContentLoaded', init)
if (window.location.hash === "#cluster") {
	var vims11 = new L.MarkerClusterGroup();  	// Set up cluster group
} else {
	var vims11 = new L.LayerGroup();  	// Otherwise set up normal marker group
}
...
//Step 7) Change if(current.type ==) { vims11.addLayer(layer_marker); }
function loadMarkersToMap(data) {
  for(var i = 0; i < data.length; i++) {
    ...
    if(current.type == '11') { vims11.addLayer(layer_marker); }
	}
