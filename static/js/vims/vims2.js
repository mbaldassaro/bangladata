...
/*Step 3) Change: vims2 */
var vims2;  //declared outside function for use in layer control
window.addEventListener('DOMContentLoaded', init)
if (window.location.hash === "#cluster") {
	var vims2 = new L.MarkerClusterGroup();  	// Set up cluster group
} else {
	var vims2 = new L.LayerGroup();  	// Otherwise set up normal marker group
}
...
//Step 7) Change if(current.type ==) { vims2.addLayer(layer_marker); }
function loadMarkersToMap(data) {
  for(var i = 0; i < data.length; i++) {
    ...
    if(current.type == '2') { vims2.addLayer(layer_marker); }
	}
