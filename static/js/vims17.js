...
/*Step 3) Change: vims17 */
var vims17;  //declared outside function for use in layer control
window.addEventListener('DOMContentLoaded', init)
if (window.location.hash === "#cluster") {
	var vims17 = new L.MarkerClusterGroup();  	// Set up cluster group
} else {
	var vims17 = new L.LayerGroup();  	// Otherwise set up normal marker group
}
...
//Step 7) Change if(current.type ==) { vims17.addLayer(layer_marker); }
function loadMarkersToMap(data) {
  for(var i = 0; i < data.length; i++) {
    ...
    if(current.type == '17') { vims17.addLayer(layer_marker); }
	}
