...
/*Step 3) Change: vims3 */
var vims3;  //declared outside function for use in layer control
window.addEventListener('DOMContentLoaded', init)
if (window.location.hash === "#cluster") {
	var vims3 = new L.MarkerClusterGroup();  	// Set up cluster group
} else {
	var vims3 = new L.LayerGroup();  	// Otherwise set up normal marker group
}
...
//Step 7) Change if(current.type ==) { vims3.addLayer(layer_marker); }
function loadMarkersToMap(data) {
  for(var i = 0; i < data.length; i++) {
    ...
    if(current.type == '3') { vims3.addLayer(layer_marker); }
	}
