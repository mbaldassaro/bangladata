...
/*Step 3) Change: vims5 */
var vims5;  //declared outside function for use in layer control
window.addEventListener('DOMContentLoaded', init)
if (window.location.hash === "#cluster") {
	var vims5 = new L.MarkerClusterGroup();  	// Set up cluster group
} else {
	var vims5 = new L.LayerGroup();  	// Otherwise set up normal marker group
}
...
//Step 7) Change if(current.type ==) { vims5.addLayer(layer_marker); }
function loadMarkersToMap(data) {
  for(var i = 0; i < data.length; i++) {
    ...
    if(current.type == '5') { vims5.addLayer(layer_marker); }
	}
