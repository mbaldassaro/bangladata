...
/*Step 3) Change: vims7 */
var vims7;  //declared outside function for use in layer control
window.addEventListener('DOMContentLoaded', init)
if (window.location.hash === "#cluster") {
	var vims7 = new L.MarkerClusterGroup();  	// Set up cluster group
} else {
	var vims7 = new L.LayerGroup();  	// Otherwise set up normal marker group
}
...
//Step 7) Change if(current.type ==) { vims7.addLayer(layer_marker); }
function loadMarkersToMap(data) {
  for(var i = 0; i < data.length; i++) {
    ...
    if(current.type == '7') { vims7.addLayer(layer_marker); }
	}
