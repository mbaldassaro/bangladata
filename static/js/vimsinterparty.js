...
function init() {
  Tabletop.init({
    key: 'https://docs.google.com/spreadsheets/d/10hhs6zEFTBSXtTbbaUuIjOyPoQdgo6yxA2a5QRck9hQ/pubhtml',
      callback: function(data, tabletop) { console.log(data) },
      simpleSheet: true } )
}

/*Step 3) Change: vimsinterparty */
var vimsinterparty;  //declared outside function for use in layer control
window.addEventListener('DOMContentLoaded', init)
if (window.location.hash === "#cluster") {
	var vimsinterparty = new L.MarkerClusterGroup();  	// Set up cluster group
} else {
	var vimsinterparty = new L.LayerGroup();  	// Otherwise set up normal marker group
}

//Step 4) Same
var spreadsheet_key = '10hhs6zEFTBSXtTbbaUuIjOyPoQdgo6yxA2a5QRck9hQ';
...
//Step 7) Change if(current.type ==) { vimsinterparty.addLayer(layer_marker); }
function loadMarkersToMap(data) {
  for(var i = 0; i < data.length; i++) {
    ...
    if(current.type == '2') { vimsinterparty.addLayer(layer_marker); }
	}
