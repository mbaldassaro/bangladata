...
function init() {
  Tabletop.init({
    key: 'https://docs.google.com/spreadsheets/d/1KUawdFaUKD6SWUqOOkdBQNbXYcdU5QSl1H045O3A9v4/pubhtml',
      callback: function(data, tabletop) { console.log(data) },
      simpleSheet: true } )
}

/*Step 3) Change: vimsintraparty */
var vimsintraparty;  //declared outside function for use in layer control
window.addEventListener('DOMContentLoaded', init)
if (window.location.hash === "#cluster") {
	var vimsintraparty = new L.MarkerClusterGroup();  	// Set up cluster group
} else {
	var vimsintraparty = new L.LayerGroup();  	// Otherwise set up normal marker group
}

//Step 4) Same
var spreadsheet_key = '1KUawdFaUKD6SWUqOOkdBQNbXYcdU5QSl1H045O3A9v4';
...
//Step 7) Change if(current.type ==) { vimsintraparty.addLayer(layer_marker); }
function loadMarkersToMap(data) {
  for(var i = 0; i < data.length; i++) {
    ...
    if(current.type == '1') { vimsintraparty.addLayer(layer_marker); }
	}
